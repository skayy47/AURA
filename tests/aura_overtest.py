...
assert len(QUESTIONS_EN) == 60, f"Expected 60 EN questions, got {len(QUESTIONS_EN)}"
...
def test_dataset(csv_path: Path, ask_en: list[str], ask_fr: list[str]) -> dict:
    ...
    try:
        ...
        check("charts" in exp and len(exp["charts"]) > 0, f"charts generated: {len(exp['charts'])}", "no charts")
        results["explore"] = exp
    except Exception as e:
    ...
    try:
        ...
        # Check FR language markers
        all_titles = " ".join(i["title"] for i in an_fr["insights"])
        fr_markers = ["est ", "dans ", "de ", "sur ", "avec ", "par ", "pour "]
        has_fr = any(m in all_titles for m in fr_markers)
    ...
    for i, q in enumerate(ask_fr, 1):
        ...
        if resp.startswith("ERROR") or len(resp) < 30:
            ...
            # Check if response is actually in French
            fr_words = ["les", "des", "est", "une", "pas", "dans", "pour", "sur", "avec"]
            if any(f" {w} " in resp.lower() for w in fr_words):
                fr_in_response += 1
            if i <= 3:
    ...
    check(fr_in_response >= len(ask_fr) * 0.8,
          f"FR responses: {fr_in_response}/{len(ask_fr)} contain French",
          f"FR responses lack French: only {fr_in_response}/{len(ask_fr)}")
